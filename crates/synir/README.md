...
The default dependency graph includes only `synir-core`; both crates are `no_std` and need no allocator. Optional `proc-macro` and `quote` features select host boundaries. `alloc` reserves the optional allocation profile. These flags currently wire the scaffold only.
...
Point a local workspace dependency at `crates/synir` to inspect the scaffold.
