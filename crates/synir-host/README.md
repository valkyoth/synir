...
The optional compiler-host boundary for Synir.
...
A host library, separate from the portable core. Future native token import/export and span storage must preserve invocation ownership. It currently depends only on `synir-core` and exposes no compiler adapter API.
...
Point a local workspace dependency at `crates/synir-host` to inspect the scaffold.
